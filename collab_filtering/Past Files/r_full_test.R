...
################# model evaluation ################# 
cat("\nNow evaluating model accuracy using collaborative filtering...\n")

library(purrr)

user_counts <- ratings %>% group_by(Username) %>% tally()
active_users <- user_counts %>% filter(n >= 10) %>% pull(Username)

evaluate_user_predictions <- function(user_id) {
  # Grab user-specific test data
  test_data <- ratings_test %>% filter(Username == user_id)
  
  # Grab user ratings from training matrix
  if (!(user_id %in% rownames(Rmat))) return(NULL)
  test_vec <- Rmat[user_id, ]
  
  # Which items this user rated
  rated_items <- which(test_vec != 0)
  if (length(rated_items) == 0) return(NULL)
  
  # Predict function: now defensive
  predict_rating <- function(item_id) {
    if (!(item_id %in% colnames(Rmat))) return(NA)
    
    sims <- sapply(rated_items, function(r_item) {
      if (!(r_item %in% colnames(Rmat))) return(0)
      cosine_sim_sp(Rmat[, item_id], Rmat[, r_item])
    })
    
    # Handle division by zero
    if (sum(abs(sims)) == 0) return(0)
    
    return(sum(sims * test_vec[rated_items]) / sum(abs(sims)))
  }
  
  print(test_data)
  
  # Apply predictions
  predictions <- sapply(test_data$BGGId, predict_rating)
  
  return(tibble(
    UserId = user_id,
    BGGId = test_data$BGGId,
...
# Make sure ratings_test is defined!
# Example if you're splitting data earlier:
ratings_split <- initial_split(ratings, prop = 0.8)
ratings_train <- training(ratings_split)
ratings_test <- testing(ratings_split)

all_predictions <- map_dfr(sample_users, evaluate_user_predictions)
...
rmse <- sqrt(mean((all_predictions$Predicted - all_predictions$Rating)^2, na.rm = TRUE))
print(rmse)

# Optional: Plot
library(ggplot2)
ggplot(all_predictions, aes(x = Rating, y = Predicted)) +
