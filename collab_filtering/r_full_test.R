...
predictions <- sapply(candidate_items, function(item) {
  # for each game TestUser rated, get similarity with candidate item
  sims <- sapply(rated_items, function(r_item) {
    cosine_sim_sp(Rmat[, item], Rmat[, r_item])
  })
  # if all similarities are zero, null / error catch
...
# remove items with NA ratings / error catch
...
predict_user_ratings <- function(user_id) {
  ...
  user_ratings <- Rmat_train[user_id, ]
  rated_items <- which(user_ratings != 0)
  
  if (length(rated_items) == 0) {
    cat("User", user_id, "has no ratings in training data\n")
    return(NULL)
  }
  ...
  # predict 
  predictions <- sapply(test_items, function(item) {
    sims <- sapply(rated_items, function(r_item) {
      tryCatch({
        cosine_sim_sp(Rmat_train[, item], Rmat_train[, r_item])
      }, error = function(e) {
        return(0)  
      })
    })
    ...
    sum(sims * user_ratings[rated_items]) / sum(abs(sims))
  })
