...
class MultiBranchGameEncoder(nn.Module):
    def __init__(self, groups_dims, fusion_out_dim=256,
        ...
        self.out_dims = {}
        # Increase dropout rates here to 0.3 for branches.
        ...
        # Increase dropout in fusion to 0.4.
...
##############################################
# Rating Predictor with Heteroscedastic Regression
##############################################
class RatingPredictor(nn.Module):
    def __init__(self, game_encoder, num_users, num_games, latent_dim=256,
        ...
        # Increase dropout in the MLP to 0.5.
...
def train_model(model, num_epochs, train_loader, test_loader, optimizer, criterion, device='cpu', patience=3):
    ...
    for epoch in tqdm(range(num_epochs), desc="Training Epochs"):
        ...
        if te_loss < best_loss:
            best_loss = te_loss
            epochs_no_improve = 0
            torch.save(model.state_dict(), "ncf_model_hetero.pth")
        else:
            epochs_no_improve += 1
...
def plot_predictions(true, pred, save_as=None, aggregate=True, shaded_region=True, smoothing_window=5, title="Predicted vs. Actual"):
...
def main():
    ...
    unique_users = unique_users # [1000:1500]  # adjust as needed
    ...
    # Set heteroscedastic loss; note that normalized ratings range approximately from -3 to 3.
    criterion = HeteroscedasticLoss(rating_to_weight, min_rating=-3.0, max_rating=3.0, precision=2).to(device)
    num_epochs = 15
    patience = 3
